...
def score_job(
    ...
    *,
    ...
    # Profile alignment check: penalize when user themes don't match the JD.
    # Penalties are intentionally strong — senior S&O roles score 78-82 from
    # keyword groups alone, so mild deductions are invisible against that base.
    if user_profile is not None:
        ...
        if len(_align_themes) >= 2:
            ...
            if _ratio == 0:
                # No themes match at all — clear background mismatch
                score -= 22
                deductions["profile_theme_mismatch"] = -22
                print(f"[SCORING] profile_theme_mismatch: 0/{len(_align_themes)} themes matched, -22")
            elif _ratio < 0.25:
                # Only metadata / incidental matches (e.g. "financial services" in footer)
                score -= 15
                deductions["profile_theme_mismatch"] = -15
                print(f"[SCORING] profile_theme_mismatch: {_matching}/{len(_align_themes)} themes matched, -15")
            elif _ratio < 0.5:
                # Partial overlap — some alignment but not enough to call it a fit
                score -= 6
                deductions["profile_theme_mismatch"] = -6
                print(f"[SCORING] profile_theme_mismatch: {_matching}/{len(_align_themes)} themes matched, -6")
